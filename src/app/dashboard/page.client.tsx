...
type DashboardData = AnalyticsSnapshot & { demo: boolean; dataSource: "REAL" | "DEMO" | "EMPTY"; realOrderCount: number };
...
const displayNumber = (value: number | null, unit: Row["unit"]) => value === null ? "—" : unit === "percent" ? `${(value * 100).toFixed(1)}%` : `¥${Number(value).toFixed(2)}`;
...
export default function Dashboard({ user }: { user: SessionUser }) {
  ...
  const differenceOutcome = selected?.difference === null || selected?.difference === 0 ? "neutral" : selected && selected.difference > 0 ? "good" : "risk";

  return <main className="analytics-page" id="workspace-content">
    <PageHeader eyebrow="竞争价格力 / Analytics" title="竞争价格力分析" description="从已确认的双平台采集数据中，快速定位平台差异与待跟进商家。" backHref="/" actions={<><span className="analytics-data-badge">{data.dataSource === "DEMO" ? "演示数据预览" : data.dataSource === "REAL" ? "正式数据" : "暂无正式数据"}</span><button type="button" className="analytics-reset" onClick={reset}>重置筛选</button></>} />
    ...
    {loadState === "ready" ? <>
      <section className="analytics-kpi-grid" aria-label="当前分析结论">
        ...
        <MetricCard label={`美团 · ${metricLabels[filter.metric]}`} value={selected ? displayNumber(selected.meituan, selected.unit) : "—"} description={data.platforms.MEITUAN.validOrderCount ? `${data.platforms.MEITUAN.validOrderCount} 条平均` : "暂无美团观测"} tone="meituan" metric={filter.metric} />
        <MetricCard label={`B家 · ${metricLabels[filter.metric]}`} value={selected ? displayNumber(selected.bJia, selected.unit) : "—"} description={data.platforms.B_JIA.validOrderCount ? `${data.platforms.B_JIA.validOrderCount} 条平均` : "暂无 B 家观测"} tone="bjia" metric={filter.metric} />
        <MetricCard label="平台差异" value={selected?.difference === null || !selected ? "—" : `${selected.difference > 0 ? "+" : ""}${displayNumber(selected.difference, selected.unit)}`} description={selected?.difference === null ? "需同时具备两个平台观测" : "美团 − B家；正负仅表示数值差异"} tone="outcome" outcome={differenceOutcome} />
      </section>
