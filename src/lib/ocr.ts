...
export async function recognizeOrderScreenshot(imageUrl: string): Promise<RecognitionResult> {
  ...
  const response = await fetch("https://apihub.agnes-ai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: process.env.AGNES_MODEL || "agnes-2.0-flash", temperature: 0, max_tokens: 1200, messages: [{ role: "system", content: extractionPrompt }, { role: "user", content: [{ type: "text", text: "请识别这张订单详情截图，并严格按要求返回 JSON。" }, { type: "image_url", image_url: { url: imageUrl } }] }] }),
  });
