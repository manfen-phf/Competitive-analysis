...
describe("Cloudflare runtime secrets", () => {
  it("reads administrator and Qwen secrets through the runtime binding helper", () => {
    ...
    expect(ocr).toContain('getRuntimeSecret("QWEN_API_KEY")');
    expect(ocr).toContain('getRuntimeSecret("QWEN_MODEL")');
    expect(ocr).not.toContain("AGNES_");
  });
});
